# This script checks the pairings of the image and the annotation file match.

# usage : python pair_files.py data_dir
...
def match_files(data_dir):
    ...
    for file in files:
        ...
        if (fileExtension == '.jpg' or fileExtension == '.JPG' or fileExtension == '.png' 
            or fileExtension == '.PNG' or fileExtension == '.jpeg'):
            total_img += 1
            txt_path = data_dir + '/' + fileName + '.txt'
            if not os.path.isfile(txt_path):
                truckfile = open(txt_path, 'w')
                truckfile.write()
                truckfile.close()
